...
static counter_t sim_1cycle_stall_q1 = 0;
static counter_t sim_2cycle_stall_q1 = 0;

static counter_t sim_1cycle_stall_q2 = 0;
static counter_t sim_2cycle_stall_q2 = 0;
...
sim_reg_stats(struct stat_sdb_t *sdb) {
    ...
    stat_reg_counter(sdb, "sim_1cycle_stall_q1",
                     "total number of 1 cycle stall (q1)",
                     &sim_1cycle_stall_q1, sim_1cycle_stall_q1, NULL);
    stat_reg_counter(sdb, "sim_2cycle_stall_q1",
                     "total number of 2 cycle stall (q1)",
                     &sim_2cycle_stall_q1, sim_2cycle_stall_q1, NULL);
    stat_reg_counter(sdb, "sim_1cycle_stall_q2",
                     "total number of 1 cycle stall (q2)",
                     &sim_1cycle_stall_q2, sim_1cycle_stall_q2, NULL);
    stat_reg_counter(sdb, "sim_2cycle_stall_q2",
                     "total number of 2 cycle stall (q2)",
                     &sim_2cycle_stall_q2, sim_2cycle_stall_q2, NULL);
    ...
    stat_reg_formula(sdb, "CPI_from_RAW_hazard_q1",
                     ...
                     "1+sim_1cycle_stall_q1/sim_num_insn+sim_2cycle_stall_q1*2/sim_num_insn", NULL);

    stat_reg_formula(sdb, "CPI_from_RAW_hazard_q2",
                     "CPI from RAW hazard (q2)",
                     "1+sim_1cycle_stall_q2/sim_num_insn+sim_2cycle_stall_q2*2/sim_num_insn", NULL);
...
sim_main(void) {
    /* ECE552 Assignment 1 - BEGIN CODE */
    int r_out[2], r_in[3];
    int is_I1_access_mem = 0;
    int is_I2_access_mem = 0;
    int c1_out = DNA;
    int c2_out = DNA;
    /* ECE552 Assignment 1 - END CODE */
    md_inst_t inst;
...
#include "machine.def"
        ...
        for (i = 0; i < 3; i++) {
            if (r_in[i] != DNA) {
                if (r_in[i] == c2_out && is_I2_access_mem) {
                    if((MD_OP_FLAGS(op) & F_STORE) && i == 0){				
				break;
			}
			else{
				stall_q2=1;
        ...
        is_I2_access_mem = is_I1_access_mem;
